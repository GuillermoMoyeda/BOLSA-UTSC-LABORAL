...
function getUserDocId() {
    if (currentSession?.docId) return String(currentSession.docId).trim();
    if (currentSession?.correo) return String(currentSession.correo).trim();
    if (currentSession?.email) return String(currentSession.email).trim();
    if (userDocId) return String(userDocId).trim();
    if (currentSession?.id && isNaN(Number(currentSession.id)) === false) {
        return null;
    }
    return currentSession?.id ? String(currentSession.id).trim() : null;
}

document.addEventListener('DOMContentLoaded', async () => {
    currentSession = getCurrentSession();
    if (!currentSession || currentSession.rol !== 'reclutador') return;
    userDocId = currentSession.id || currentSession.correo || currentSession.email || null;
...
function renderProfile() {
    ...
    if (recruiterData.logoUrl) {
        ...
        const cachedLogo = localStorage.getItem(`companyLogoUrl:${getUserDocId()}`);
        if (cachedLogo) document.getElementById('displayLogo').src = cachedLogo;
    }
    if (recruiterData.bannerUrl) {
        document.getElementById('displayBanner').src = recruiterData.bannerUrl;
    } else {
        const cachedBanner = localStorage.getItem(`companyBannerUrl:${getUserDocId()}`);
        if (cachedBanner) document.getElementById('displayBanner').src = cachedBanner;
    }
...
async function executeSaveProfile() {
    ...
    try {
        const docId = getUserDocId();
        if (!docId) throw new Error('ID de usuario inválido');
        await db.collection('usuarios').doc(docId).update(updatedData);
...
async function handleUpload(file, field, displayId) {
    ...
    try {
        ...
        const docId = getUserDocId();
        if (!docId) throw new Error('ID de usuario inválido');
        const fileRef = storageRef.child(`empresas/${docId}/${field}_${Date.now()}`);

        await fileRef.put(file);
        const downloadURL = await fileRef.getDownloadURL();

        await db.collection('usuarios').doc(docId).update({ [field]: downloadURL });
        document.getElementById(displayId).src = downloadURL;
        if (field === 'logoUrl') localStorage.setItem('companyLogoUrl:' + docId, downloadURL);
        if (field === 'bannerUrl') localStorage.setItem('companyBannerUrl:' + docId, downloadURL);
        recruiterData = { ...recruiterData, [field]: downloadURL };
