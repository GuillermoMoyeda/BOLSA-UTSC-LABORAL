...
function renderList() {
    ...
    container.innerHTML = page.map(v => {
        ...
        const badgePost = v.isPostulado
            ? `<span class="exp-badge-postulado"><i class="fas fa-check-circle"></i> Postulado</span>`
            : '';
        ...
        return `
            <div class="exp-vac-card ${currentSelectedId === v.id ? 'active' : ''}" onclick="selectVacancy('${v.id}')">
...
async function postularA(vacanteId) {
    try {
        ...
        if (q.empty || !q.docs[0].data().cvUrl) {
            ...
            setTimeout(() => { window.location.href = '/alumno/mi-perfil.html'; }, 3000);
            return;
        }
