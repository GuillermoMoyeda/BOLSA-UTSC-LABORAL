// ==========================================
// LOGICA VISTA ESTUDIANTE: EXPLORAR VACANTES
// ==========================================
...
export async function initAlumnoPrincipal() {
...
async function fetchVacancies() {
  try {
    ...
    if (typeof window !== 'undefined') {
      window.allVacantes = allVacantes;
    }
    const counter = document.getElementById('vacantesCountLabel');
    if (counter) counter.textContent = `${filteredVacantes.length} vacantes`;
...
function handleSearch() {
  ...
  currentPage = 1;
  const counter = document.getElementById('vacantesCountLabel');
  if (counter) counter.textContent = `${filteredVacantes.length} resultados`;
  renderList();
}
...
function renderList() {
  ...
  if (filteredVacantes.length === 0) {
    ...
    paginatedItems.forEach(v => {
      ...
      const badgePostulado = v.isPostulado ? `<div class="badge-postulado-mini"><i class="fas fa-check-circle"></i> Postulado</div>` : '';
...
function updatePagination() {
  ...
  if (totalPages > 1) {
    ...
    document.getElementById('pageIndicator').textContent = `Pagina ${currentPage} de ${totalPages}`;
...
async function selectVacancy(id) {
  ...
  const detailBody = document.getElementById('innerViewBody');
    ...
    if (postDoc.exists) {
      postularHtml = `
                <div class="postular-wrap already-sent" style="text-align: center;">
                    ...
                    <p style="margin:0; font-size:0.85rem; color:#64748b; text-align: center;">La empresa pronto revisara tu perfil y CV.</p>
                </div>
            `;
    } else {
      postularHtml = `
                <div class="postular-wrap">
                    ...
                    <p style="margin:0 0 15px; font-size:0.85rem; color:#475569;">Al postularte, la empresa tendra acceso directo a tu Curriculo Vitae y datos de contacto.</p>
            ...
            <div class="vacancy-detail-pro" style="box-shadow:none; border-radius:0;">
                ...
                <div class="vacancy-main-grid">
                    <div class="main-info">
                        ...
                        <div class="detail-section">
                            <h4><i class="fas fa-tools"></i> Requisitos Tecnicos / Blandos</h4>
                    ...
                    <div class="sidebar-data">
                        ...
                        <div class="data-card deadline">
                            <label><i class="fas fa-hourglass-half"></i> Fecha Limite</label>
...
async function postularA(vacanteId) {
  try {
    ...
    if (q.empty || !q.docs[0].data().cvUrl) {
      showToast('Atencion! Para postularte necesitas subir tu CV primero. Seras redirigido.', 'error');
      setTimeout(() => {
        window.location.href = '/alumno/mi-perfil.html';
      }, 3000);
      return;
    }
    ...
    console.error(e);
    showToast('Hubo un error al procesar tu postulacion.', 'error');
    const btn = document.getElementById('btnPostularAction');
    if (btn) {
      btn.disabled = false;
...
console.log('Alumno principal cargado');
